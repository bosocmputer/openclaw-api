...
function normalizeConversationMedia(media) {
  if (!media || typeof media !== 'object') return null
  const rawId = media.id == null ? null : String(media.id)
  const id = rawId && /^[a-f0-9]{32,64}$/i.test(rawId) ? rawId : null
  const rawPreviewUrl = media.previewUrl == null ? null : String(media.previewUrl)
  const previewUrl = rawPreviewUrl && /^\/api\/monitor\/media\/[a-f0-9]{32,64}$/i.test(rawPreviewUrl)
    ? rawPreviewUrl
    : id ? `/api/monitor/media/${id}` : undefined
  ...
  return {
    ...
    kind,
    mimeType,
    fileName: media.fileName || media.file_name || media.filename || media.name
    ...
    caption: media.caption || media.text || media.alt ? redactText(String(media.caption || media.text || media.alt), 500) : undefined,
    hasPreview: Boolean(media.hasPreview && (id || previewUrl)),
    previewUrl,
  }
}

function normalizeConversationMediaList(mediaList) {
  if (!Array.isArray(mediaList)) return []
  const seen = new Set()
  const out = []
  for (const raw of mediaList.slice(0, 20)) {
    const item = normalizeConversationMedia(raw)
    if (!item) continue
...
function turnToEvents(turn) {
  ...
  const media = normalizeConversationMediaList(turn.media)
  events.push({
    type: 'user',
...
async function getConversationDetail(turnId) {
  ...
  return {
    turn: rowToTurn(turnRes.rows[0], issues, events),
