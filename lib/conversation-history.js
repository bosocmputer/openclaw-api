...
function turnToEvents(turn) {
  ...
  events.push({
    ...
    payload: {
      ...
      user: turn.user,
      media: Array.isArray(turn.media) ? turn.media.map(media => ({
        kind: media.kind,
        mimeType: media.mimeType,
        fileName: media.fileName,
        sizeBytes: media.sizeBytes,
        caption: media.caption,
        hasPreview: Boolean(media.hasPreview),
      })) : [],
      mediaCount: Number.isFinite(Number(turn.mediaCount)) ? Math.max(0, Number(turn.mediaCount)) : Array.isArray(turn.media) ? turn.media.length : 0,
    },
  })
