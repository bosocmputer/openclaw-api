...
function hasDerivedFilters(filters) {
  return Boolean(filters.issueTag || filters.reviewTarget || filters.hasToolError || filters.slowOnly)
}
...
function extractCandidates(event) {
  const payload = safeJson(event.payload) || {}
  const candidates = []
  const arrays = collectArrays(payload, /^(candidates|items|products|results|rows|data)$/i)
...
function deriveIssues(turnLike, rawEvents = []) {
  ...
  for (const event of events) {
    ...
    const statusText = collectValues(payload, /status|state|error|reason/i).map(String).join(' ')
    ...
    const scores = extractConfidenceScores(payload)
    const maxConfidence = scores.length ? Math.max(...scores) : null
    const noResult = /no[_ -]?result|not[_ -]?found|ไม่พบสินค้า|ไม่พบข้อมูล|search yielded no results/i.test(lower)
      || (/candidates|results|items|products/i.test(lower) && candidates.length === 0 && /0|empty|\[\]/i.test(lower))
    const needsRefine = /needs[_ -]?refine|ambiguous|refine keyword|ระบุ.*เพิ่ม|คำค้น.*เพิ่ม|กรุณาระบุ|กรุณาเลือก/i.test(lower)
...
async function enrichRowsWithIssues(rows) {
  ...
  return rows.map(row => {
    const issues = deriveIssues(row, eventsByTurnId.get(row.turn_id) || [])
    return rowToTurn(row, issues)
  })
}

function applyDerivedFilters(turns, filters) {
  return turns.filter(turn => {
    if (filters.issueTag && !turn.issueTags.includes(filters.issueTag)) return false
...
async function getConversationDetail(turnId) {
  ...
  return {
    turn: rowToTurn(turnRes.rows[0], issues),
...
async function exportConversations(rawFilters, format, actor) {
  ...
  if (mode === 'codex_review_pack') {
    return {
      ...
      body: toCodexReviewPack(turns, filters, eventsByTurnId),
    }
  }
  if (mode === 'issues_csv') {
...
function toIssuesCsv(turns) {
  const cols = [
    ...
    'review_targets', 'issue_tags', 'evidence', 'keyword', 'model', 'duration_ms',
...
function toCodexReviewPack(turns, filters, eventsByTurnId) {
