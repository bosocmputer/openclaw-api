...
const systemObservability = require('./system-observability')
...
function exportPseudonym(value, prefix = 'subject') {
  return `[${prefix}:${crypto.createHash('sha256').update(String(value)).digest('hex').slice(0, 10)}]`
}

function redactExportIdentifiers(value) {
  return String(value || '')
    .replace(/\bU[0-9a-f]{32}\b/giu, match => exportPseudonym(match))
    .replace(/(\[(?:Telegram|LINE)\s+(?:user|chat|sender)\s*:\s*)([^\]\s]+)/giu, (_match, prefix, id) => `${prefix}${exportPseudonym(id)}`)
    .replace(/((?:VIN|เลขตัวถัง|chassis)\s*[:#=\-]?\s*)[A-HJ-NPR-Z0-9]{17}\b/giu, '$1[redacted-vin]')
    .replace(/((?:ทะเบียน|license\s+plate|plate\s+number|plate\s+reading)\s*[:#=\-]?\s*)["']?[^\n,.;"']+?["']?(?=\s+(?:path|media|vin|chassis)\s*[:=]|[,.;\n]|$)/giu, '$1[redacted-plate]')
    .replace(/(?:media:\/\/[^\s)\]}]+|\/(?:root|home|Users|var|tmp)\/[^\s)\]}]+)/gu, '[redacted-path]')
}
...
function redactExportText(text, max = 12000) {
  return truncateText(redactExportIdentifiers(redactText(text, max)), max) || ''
}

function redactExportValue(value, depth = 0) {
  if (depth > 8) return '[max-depth]'
  if (value === null || value === undefined) return value
  if (typeof value === 'string') return redactExportText(value, 5000)
  if (value instanceof Date) return value.toISOString()
  if (typeof value !== 'object') return value
  if (Array.isArray(value)) return value.slice(0, 80).map(item => redactExportValue(item, depth + 1))
  const out = {}
  for (const [key, raw] of Object.entries(value)) {
    out[key] = /token|api[_-]?key|authorization|password|secret|botToken|channelAccessToken|refreshToken|accessToken|cookie/i.test(key)
      ? '[redacted]'
      : redactExportValue(raw, depth + 1)
  }
  return out
}

function sanitizeTurnForExport(turn) {
  const sanitized = redactExportValue(turn)
  return {
    ...sanitized,
    id: exportPseudonym(turn.id, 'turn'),
    ...(turn.sessionKey ? { sessionKey: exportPseudonym(turn.sessionKey, 'session') } : {}),
    ...(turn.chatUser && turn.chatUser !== 'unknown'
      ? { chatUser: exportPseudonym(turn.chatUser) }
      : {}),
  }
}
...
function parseQueryFilters(query = {}) {
  ...
  return {
    ...
    learningDecision: query.learningDecision ? String(query.learningDecision) : null,
...
function buildWhere(filters, params, alias = 't') {
  ...
  if (filters.learningDecision) {
    const value = String(filters.learningDecision)
    if (value === 'promotable') {
      clauses.push(`EXISTS (
        SELECT 1 FROM memory_observations mo
        WHERE mo.source_turn_id = ${alias}.turn_id
          AND mo.status = 'observed'
          AND mo.recommended_action = 'policy_promote'
          AND mo.risk IN ('low','medium')
      )`)
    } else if (value === 'blocked') {
      clauses.push(`EXISTS (
        SELECT 1 FROM memory_observations mo
        WHERE mo.source_turn_id = ${alias}.turn_id
          AND (mo.risk = 'high' OR mo.recommended_action = 'block_truth')
      )`)
    } else if (value === 'mcp_search_review') {
      clauses.push(`EXISTS (
        SELECT 1 FROM memory_observations mo
        WHERE mo.source_turn_id = ${alias}.turn_id
          AND mo.recommended_action = 'mcp_search_review'
      )`)
    } else if (value === 'search_hint') {
      clauses.push(`EXISTS (
        SELECT 1 FROM memory_observations mo
        WHERE mo.source_turn_id = ${alias}.turn_id
          AND (mo.type = 'search_hint' OR mo.recommended_action = 'search_hint_candidate')
      )`)
    } else if (value === 'description_suggestion') {
      clauses.push(`EXISTS (
        SELECT 1 FROM memory_observations mo
        WHERE mo.source_turn_id = ${alias}.turn_id
          AND (mo.type = 'description_suggestion' OR mo.recommended_action = 'description_suggestion')
      )`)
    } else if (value === 'agent_brain') {
      clauses.push(`EXISTS (
        SELECT 1 FROM memory_observations mo
        WHERE mo.source_turn_id = ${alias}.turn_id
      )`)
      ...
      clauses.push(`EXISTS (
        SELECT 1 FROM memory_observations mo
        WHERE mo.source_turn_id = ${alias}.turn_id
          AND (mo.risk = 'high' OR mo.recommended_action IN ('block_truth','blocked_noise','manual_review'))
      )`)
    }
  }
...
async function ensureMemorySchemaForFilters(filters) {
  if (filters?.learningDecision && memoryAuto.isAvailable()) {
    await memoryAuto.ensureSchema()
  }
}

async function fetchTurnRows(filters, options = {}) {
  await ensureMemorySchemaForFilters(filters)
  const params = []
...
async function exportConversations(rawFilters, format, actor) {
  ...
  const exportTurns = turns.map(sanitizeTurnForExport)
  const exportEventsByTurnId = new Map(exportTurns.map((turn, index) => [
    turn.id,
    (eventsByTurnId.get(turns[index].id) || []).map(event => redactExportValue(event)),
  ]))
  ...
  if (mode === 'codex_review_pack') {
    return {
      ...
      body: await toCodexReviewPack(exportTurns, filters, exportEventsByTurnId),
    }
  }
  if (mode === 'issues_csv') {
    return { contentType: 'text/csv; charset=utf-8', filename: 'conversation-issues.csv', body: toIssuesCsv(exportTurns) }
  }
  if (mode === 'events_jsonl') {
    return { contentType: 'application/x-ndjson; charset=utf-8', filename: 'conversation-events.jsonl', body: toEventsJsonl(exportTurns, exportEventsByTurnId) }
  }
  if (mode === 'learning_review_pack') {
    return {
      contentType: 'text/markdown; charset=utf-8',
      filename: 'conversation-learning-review-pack.md',
      body: toLearningReviewPack(exportTurns, filters, exportEventsByTurnId),
    }
  }

  if (format === 'csv') return { contentType: 'text/csv; charset=utf-8', filename: 'conversation-history.csv', body: toCsv(exportTurns) }
  if (format === 'jsonl') return { contentType: 'application/x-ndjson; charset=utf-8', filename: 'conversation-history.jsonl', body: exportTurns.map(turn => JSON.stringify(turn)).join('\n') + '\n' }
  if (format === 'markdown') return { contentType: 'text/markdown; charset=utf-8', filename: 'conversation-history.md', body: toMarkdown(exportTurns) }
...
function shortMemory(memory) {
  return {
    ...
    content: redactExportText(memory.content || '', 360),
  }
}
...
function profileSummary(profile) {
  return {
    ...
    summary: redactExportText(profile.summary || '', 360),
    soulBlockHash: profile.soulBlockHash || null,
    soulBlockChars: String(profile.soulBlock || '').length,
  }
}

async function buildCodexContextSnapshot(turns, filters) {
  const warnings = []
  let config = null
  try {
  ...
  try {
    ...
    const observations = await Promise.all(agentIds.map(agentId => memoryAuto.listObservations({
      agentId,
      sourceFrom: filters?.from,
      sourceTo: filters?.to,
      limit: 120,
    ...
    recentObservations = observations.flat().slice(0, 120).map(observation => ({
      ...
      safeToPromote: observation.safeToPromote,
	      blockedReason: observation.blockedReason,
	      sourceTurnId: observation.sourceTurnId ? exportPseudonym(observation.sourceTurnId, 'turn') : null,
	      summary: redactExportText(observation.summary || '', 360),
	      evidence: redactExportValue(observation.evidence || {}),
	    }))
  } catch (err) {
  ...
  try {
    observability = await systemObservability.getObservability({ refresh: true })
  } catch (err) {
    warnings.push(`System observability snapshot unavailable: ${err.message}`)
  }
...
function appendCodexContextSnapshot(lines, snapshot) {
  ...
  lines.push('')
  if (snapshot.observability) {
    lines.push('### System Observability Snapshot', '')
    pushJsonBlock(lines, snapshot.observability)
    lines.push('')
  }
...
function appendMemoryDataQuality(lines, memorySnapshot) {
  const summaryByAgent = memorySnapshot?.summaryByAgent || {}
  lines.push('## Memory Data Quality', '')
  lines.push('Use this as tenant-specific evidence only. Do not convert examples from one customer into shared global business logic.')
  lines.push('')
  const rows = Object.entries(summaryByAgent)
  if (!rows.length) {
    lines.push('- No structured memory summary was available in this export.', '')
    return
  }
  for (const [agentId, summary] of rows) {
    const health = summary.memoryHealth || {}
    lines.push(`- ${agentId}: active ${summary.activeMemoryCount || 0}, soft ${summary.softMemoryCount || 0}, blocked ${summary.blockedCount || 0}, injected ${health.injectedChars || summary.estimatedInjectedChars || 0}/${summary.maxContextChars || 0} chars, duplicates ${health.duplicateCount || 0}, noise ${health.noiseCount || 0}, dynamic facts ${health.dynamicFactCount || 0}, vague teachings ${health.vagueTeachingCount || 0}, active not injected ${health.activeButNotInjectedCount || 0}`)
  }
  lines.push('')
  lines.push('Memory interpretation notes:')
  lines.push('- Active memory is used as runtime context only when it fits the per-agent context budget.')
  lines.push('- Observations, issue tags, media placeholders, delivery markers, and failed searches are evidence, not truth.')
  lines.push('- Price, stock, cost, availability, customer credit, special price, and substitute-product facts must come from MCP/tools.')
  lines.push('')
}

function appendAgentBrainEvidence(lines, memorySnapshot) {
  const observations = Array.isArray(memorySnapshot?.recentObservations) ? memorySnapshot.recentObservations : []
  lines.push('## Agent Brain Evidence', '')
  lines.push('Agent Brain items are tenant-scoped evidence. Search hints and SML description suggestions must be verified by MCP/SML before becoming business truth.')
  lines.push('')
  if (!observations.length) {
    lines.push('- No recent Agent Brain observations were available.', '')
    return
  }
  const searchHints = observations
    .filter(item => item.type === 'search_hint' || item.recommendedAction === 'search_hint_candidate')
    .slice(0, 20)
  const descriptionSuggestions = observations
  ...
  if (searchHints.length) {
    ...
    for (const item of searchHints) {
      lines.push(`- ${item.agentId}: ${item.summary} (${item.decision || item.recommendedAction}, turn=${item.sourceTurnId || '-'})`)
    }
    lines.push('')
  }
  if (descriptionSuggestions.length) {
    lines.push('### SML Description Suggestions', '')
    for (const item of descriptionSuggestions) {
      lines.push(`- ${item.agentId}: ${item.summary} (${item.decision || item.recommendedAction}, turn=${item.sourceTurnId || '-'})`)
    }
    lines.push('')
  }
  if (!searchHints.length && !descriptionSuggestions.length) {
    lines.push('- No search hints or description suggestions in the recent observation sample.', '')
  }
}

async function toCodexReviewPack(turns, filters, eventsByTurnId) {
  const contextSnapshot = await buildCodexContextSnapshot(turns, filters)
  ...
  appendMemoryDataQuality(lines, contextSnapshot.memory)
  appendAgentBrainEvidence(lines, contextSnapshot.memory)
  ...
  for (const tag of ISSUE_TAGS) {
    ...
    for (const turn of examples) {
      ...
      lines.push('User:')
      lines.push('')
      lines.push(`> ${redactExportText(turn.userText || '(empty)', 1200).replace(/\n/g, '\n> ')}`)
      lines.push('')
      lines.push('Assistant:')
      lines.push('')
      lines.push(`> ${redactExportText(turn.finalText || '(no final reply)', 1200).replace(/\n/g, '\n> ')}`)
      lines.push('')
    }
  }
  return lines.join('\n')
...
function toLearningReviewPack(turns, filters, eventsByTurnId) {
  ...
  for (const turn of selected.slice(0, 200)) {
    ...
    lines.push('User:')
    lines.push('')
    lines.push(`> ${redactExportText(turn.userText || '(empty)', 900).replace(/\n/g, '\n> ')}`)
    lines.push('')
    lines.push('Assistant:')
    lines.push('')
    lines.push(`> ${redactExportText(turn.finalText || '(no final reply)', 900).replace(/\n/g, '\n> ')}`)
    lines.push('')
  }
  return lines.join('\n')
}
