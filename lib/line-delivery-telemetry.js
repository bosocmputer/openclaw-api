...
function eventTypeForMarker(marker) {
  if (marker.startsWith('line_burst_')) return 'line_burst'
...
function eventTextForMarker(marker, method, fallbackReason) {
  if (marker === 'line_burst_start') return 'LINE grouped window start'
  if (marker === 'line_burst_append') return 'LINE grouped event appended'
  if (marker === 'line_burst_flush') return 'LINE grouped turn flushed'
  if (marker === 'line_burst_bypass') return 'LINE grouped turn bypassed'
  if (marker === 'line_burst_error') return 'LINE grouped turn error'
...
function parseLineDeliveryEvent(line) {
  ...
  const flushReason = fields.flushReason || undefined
  ...
  return {
    ...
    mediaCount: numberValue(fields.mediaCount),
    textCount: numberValue(fields.textCount),
    waitMs: numberValue(fields.waitMs),
    agentId: 'gateway',
    user: 'line',
  }
}
