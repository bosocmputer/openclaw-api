...
const FALLBACK_ARGS = {
  ...FALLBACK_REQUIRED_ARGS,
  search_product: ['keyword', 'limit'],
}
...
function normalizeTool(raw) {
  ...
  const required = Array.isArray(inputSchema.required)
    ? inputSchema.required.filter(Boolean)
    : (Array.isArray(raw?.required) ? raw.required.filter(Boolean) : [])
  const properties = inputSchema.properties && typeof inputSchema.properties === 'object'
    ? Object.keys(inputSchema.properties)
    : (Array.isArray(raw?.args) ? raw.args.filter(Boolean) : [])
  return {
...
function fallbackTool(name) {
  return {
    ...
    args: FALLBACK_ARGS[name] || FALLBACK_REQUIRED_ARGS[name] || [],
  }
}
