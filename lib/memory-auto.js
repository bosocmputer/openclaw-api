...
function memoryCanBeRuntimeContext(memory) {
  return memory?.status === 'active' && memory.type !== 'blocked_fact'
}
...
function canAutoPromoteObservation(observation, policy) {
  ...
  if (policy.allowChatTeaching && observation.type === 'staff_instruction' && isExplicitTeaching) return true
  ...
  if (observation.risk !== 'low') {
    return false
  }
  return observation.recommendedAction === 'observe' || observation.recommendedAction === 'policy_promote'
}
...
async function syncAgentMemoryFile(agentId) {
  ...
  if (safeMemoryFilePolicyMode(policy)) {
    const { rows } = await pgPool.query(`
      ...
      LIMIT $2
    `, [safeAgent, AUTO_MEMORY_MAX_LINES])
    let used = 0
    for (const row of rows) {
      const memory = serializeMemory(row)
      if (!memoryCanBeRuntimeContext(memory)) continue
      const line = memoryRuntimeLine(memory)
      const nextUsed = used + line.length + 1
      if (nextUsed > maxContextChars) break
      lines.push(line)
      used = nextUsed
    }
  }
  const current = readFileSafe(memoryPath)
  const next = replaceAutoMemoryBlock(current, lines)
  if (next === current) {
    return { ok: true, agentId: safeAgent, memoryPath, changed: false, lines: lines.length, chars: lines.join('\n').length }
  }
  writeFileAtomic(memoryPath, next)
  return { ok: true, agentId: safeAgent, memoryPath, changed: true, lines: lines.length, chars: lines.join('\n').length }
}
