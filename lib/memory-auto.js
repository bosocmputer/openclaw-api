...
function issueToObservation(turn, issue) {
  ...
  return {
    ...
    type,
    scope: 'agent',
    summary: `${issue.label || tag}: ${truncateText(stripManagedAgentBrainContext(issue.evidence?.userPreview || turn.userText || tag), 220)}`,
    evidence: {
      source: 'issue_tag',
      issueTag: tag,
...
function teachingText(turn) {
  const text = stripManagedAgentBrainContext(turn.userText).trim()
...
function descriptionSuggestionObservation(turn) {
  const userText = stripManagedAgentBrainContext(turn.userText)
  const finalText = stripManagedAgentBrainContext(turn.finalText)
