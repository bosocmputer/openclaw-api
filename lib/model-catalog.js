...
function normalizeKilo(provider, data) {
  ...
  return items.map(m => ({
    ...
    capabilities: {
      ...(m.capabilities || {}),
      inputModalities: m.architecture?.input_modalities || m.capabilities?.inputModalities,
      outputModalities: m.architecture?.output_modalities || m.capabilities?.outputModalities,
      supportedParameters: m.supported_parameters || m.capabilities?.supportedParameters,
      maxCompletionTokens: m.top_provider?.max_completion_tokens ?? m.capabilities?.maxCompletionTokens,
    },
  })).filter(m => m.id).map(model => {
    if (model.id !== 'kilo/auto') return model
    return {
      ...model,
      name: model.name || 'Kilo Auto',
      capabilities: {
        ...model.capabilities,
        inputModalities: ['text', 'image'],
        supportedParameters: model.capabilities?.supportedParameters || ['reasoning'],
        maxCompletionTokens: model.capabilities?.maxCompletionTokens || 128000,
      },
...
function normalizeOllamaCloud(provider, data) {
  const items = Array.isArray(data) ? data : (data.models || data.data || [])
  return items.map(m => {
    const id = m.name || m.model || m.id
    const families = Array.isArray(m.details?.families) ? m.details.families : []
    return {
      id,
...
async function loadProviderModels(provider, apiKey, opts) {
  ...
  if (provider.id === 'ollama-cloud') {
    const response = await fetchJson('https://ollama.com/api/tags', {
      ...opts,
      headers: { Authorization: `Bearer ${apiKey}` },
    })
    if (!response.ok) return errorCatalog(provider, response)
    const models = normalizeOllamaCloud(provider, response.data || {})
    return readyCatalog(provider, models, {
      warnings: ['Ollama Cloud model ids must match the hosted catalog exactly; run runtime verification before saving.'],
    })
  }
