...
const MODEL_INPUTS = new Set(['text', 'image', 'video', 'audio'])
...
function providerHasCatalogModel(config, providerId, modelId) {
...
async function materializeProviderCatalogsForRefs(config, refs, opts = {}) {
  ...
  const providersToPrepare = [...new Set(parsedRefs
    ...
    .filter(ref => !providerHasCatalogModel(next, ref.providerId, ref.modelId))
    .map(ref => ref.providerId))]

  if (providersToPrepare.length === 0) {
    return { changed: false, config: next, preparedProviders: [], warnings: [] }
  }
  ...
  for (const providerId of providersToPrepare) {
    ...
    const providerConfig = {
      ...defaults,
      ...existing,
      models: mergeModelDefinitions(existing.models, modelDefinitions),
    }
  ...
  return {
    changed: preparedProviders.length > 0,
    config: next,
    preparedProviders,
    warnings,
  }
