...
function sanitizeExistingProviderModelInputs(config) {
  ...
  for (const providerId of Object.keys(PROVIDER_RUNTIME_DEFAULTS)) {
    ...
    if (providerId === 'ollama-cloud') {
      const providerPatch = {}
      if (provider.baseUrl !== defaults.baseUrl) providerPatch.baseUrl = defaults.baseUrl
      if (provider.api !== defaults.api) providerPatch.api = defaults.api
      if (Object.prototype.hasOwnProperty.call(provider, 'baseURL')) providerPatch.baseURL = undefined
      if (Object.keys(providerPatch).length) {
        nextProvider = { ...nextProvider, ...providerPatch }
    ...
    if (providerChanged) {
      providers[providerId] = { ...nextProvider, models }
      changed = true
    }
  }
