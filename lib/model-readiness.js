...
function collectWarnings(readiness) {
  ...
  for (const agent of readiness.agents) {
    if (agent.usesImageTool && agent.imageModel.configured && agent.imageModel.primary.status !== READY) {
      warnings.push({
        id: `model.image.${agent.id}.${agent.imageModel.primary.status}`,
        status: agent.imageModel.primary.status,
        summary: `${agent.id} has a separate image understanding model configured but it is not ready: ${agent.imageModel.primary.summary}`,
      })
    }
  }
