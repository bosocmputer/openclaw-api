...
async function evaluateModelRef(ref, { role = 'primary', image = false, loadCatalog }) {
  const raw = String(ref || '').trim()
  if (!raw) {
    return makeRefStatus('', role, OPTIONAL_MISSING, 'No model configured', { capability: image ? 'image' : 'text' })
  }
...
async function evaluateModelConfig(value, { loadCatalog, image = false } = {}) {
  const config = normalizeModelConfig(value)
  const primary = await evaluateModelRef(config.primary, { role: 'primary', image, loadCatalog })
  const fallbacks = await Promise.all(config.fallbacks.map((ref, index) => (
    evaluateModelRef(ref, { role: `fallback.${index + 1}`, image, loadCatalog })
  )))
  return {
    primary,
    fallbacks,
...
async function getModelReadinessForConfig(config, opts = {}) {
  ...
  const defaultModel = await evaluateModelConfig(defaults.model, { loadCatalog })
  const defaultImageModel = await evaluateModelConfig(defaults.imageModel, { loadCatalog, image: true })
  ...
  const agents = await Promise.all((config?.agents?.list || []).map(async (agent) => {
    ...
    const model = await evaluateModelConfig(hasAgentModel ? agent.model : defaults.model, { loadCatalog })
    const imageModel = await evaluateModelConfig(hasAgentImageModel ? agent.imageModel : defaults.imageModel, { loadCatalog, image: true })
    return {
      id: agent.id,
