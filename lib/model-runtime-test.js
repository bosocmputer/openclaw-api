...
const TEST_PROMPT = 'Reply exactly: OPENCLAW_MODEL_TEST_OK'
const IMAGE_TEST_PROMPT = 'Describe this image in one short sentence.'
...
function keyFingerprintForRef(ref, config = {}) {
  ...
  const key = String(config?.env?.[parsed.provider.envKey] || '')
  if (!key) return 'none'
...
function getCachedRuntimeTest({ model, capability = 'text', mode = 'gateway', config = {}, runtimeVersion } = {}) {
  ...
  if (exact && Date.now() - exact.cachedAt < CACHE_TTL_MS) return { ...exact.result, cache: { hit: true, ttlSeconds: Math.ceil((CACHE_TTL_MS - (Date.now() - exact.cachedAt)) / 1000) } }
  ...
  for (const [key, value] of runtimeTestCache.entries()) {
    ...
    if (Date.now() - value.cachedAt >= CACHE_TTL_MS) continue
    if (!latest || value.cachedAt > latest.cachedAt) latest = value
  }
  return latest ? { ...latest.result, cache: { hit: true, ttlSeconds: Math.ceil((CACHE_TTL_MS - (Date.now() - latest.cachedAt)) / 1000) } } : null
}
...
function buildResult({ ok, status, model, capability, mode, runtimeVersion, durationMs, summary, safeMessage, data, detail, cache = false }) {
  return {
    ok,
    status,
    model,
  ...
  try {
    ...
    if (!data?.ok || !outputText) {
      ...
      const result = buildResult({
        ...
        runtimeVersion,
        durationMs: commandResult.durationMs,
        summary: outputText ? failure.summary : 'Runtime test returned no text output',
        safeMessage: outputText ? failure.safeMessage : 'Runtime ทดสอบแล้วไม่พบข้อความตอบกลับ',
        data,
        detail: combined,
      })
      runtimeTestCache.set(key, { cachedAt: Date.now(), result })
...
function runtimeStatusForRef(ref, { capability = 'text', config = {} } = {}) {
  ...
  const cached = getCachedRuntimeTest({ model: ref, capability, config })
  if (!cached) {
    return {
