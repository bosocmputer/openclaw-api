...
function classifyFailure(text, timedOut = false) {
  ...
  if (/unknown provider|provider.*not.*registered|no api provider registered|unsupported provider/i.test(value) && /ollama-cloud|ollama/i.test(value)) {
    return {
      status: 'runtime_unavailable',
      summary: 'OpenClaw runtime does not support Ollama Cloud on this build',
      safeMessage: 'OpenClaw runtime นี้ยังไม่รองรับ Ollama Cloud',
    }
  }
