...
const NATIVE_MEDIA_CONTRACT_ID = 'native-media-v1'
...
function normalizeNativeCapabilities(value) {
  const items = Array.isArray(value) ? value : []
  const out = []
  for (const item of items) {
    const capability = String(item || '').trim().toLowerCase()
    if (capability === 'image' && !out.includes('image')) out.push('image')
  }
  return out
}

function buildNativeToolContract(nativeCapabilities) {
  if (!nativeCapabilities.includes('image')) return ''
  return `
## Native Tool Contract
- nativeCapabilities=${nativeCapabilities.join(',')}
- nativeMediaContract=${NATIVE_MEDIA_CONTRACT_ID}
- native image ใช้เพื่ออ่านบริบทจากรูป เช่น รุ่นสินค้า ป้าย รุ่นรถ รหัส หรือข้อความที่เห็นในภาพ
- เมื่อมีรูป ให้ใช้สิ่งที่อ่านได้เป็น keyword/context ก่อน แล้วค่อยตรวจข้อมูลธุรกิจผ่าน MCP tools ที่อนุญาต
- ห้ามสรุปราคา สต็อก รหัสสินค้า หรือการมีขายจากรูปเพียงอย่างเดียว ถ้ารูปไม่ชัดให้ถามเพิ่มสั้น ๆ`
}
...
function generateSoulTemplate(_workspace, accessMode = 'general', _mcpUrl = null, persona = 'professional', options = {}) {
  ...
  const nativeCapabilities = normalizeNativeCapabilities(options.nativeCapabilities)
...
${buildNativeToolContract(nativeCapabilities)}
...
- ถ้าคำถามทั่วไปหรือนอกขอบเขต tool ให้ตอบหรือถามเพิ่มอย่างสุภาพ ห้ามเงียบ ห้ามส่ง error กว้าง ๆ และห้ามอ้างข้อมูลธุรกิจที่ tool ไม่ยืนยัน
...
module.exports = {
  NATIVE_MEDIA_CONTRACT_ID,
  buildNativeToolContract,
  generateSoulTemplate,
