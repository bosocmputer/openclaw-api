...
- ใช้สรรพนาม "ผม" และลงท้ายด้วย "ครับ" เสมอ`,
  ...
  const hasPriceTool = tools.includes('get_product_price')
  const toolUsage = [
    ...
    hasPriceTool
      ...
      : '- ถ้า user ขอราคาขาย แต่ Tools ที่ใช้ได้ไม่มี get_product_price ให้ค้นหาสินค้าก่อนเพื่อยืนยัน code/name แล้วตอบว่า agent นี้ยังไม่มีสิทธิ์ตรวจราคา ให้ติดต่อฝ่ายขายหรือให้ admin เปลี่ยน MCP access mode เป็น sales ห้ามเรียก get_product_price',
    ...
    '- ถ้า tool มี error ว่า not found หรือไม่มี tool นั้น ให้หยุด retry ทันที แล้วตอบข้อจำกัดแบบสุภาพ',
    '- ถ้า tool มี error อื่น ๆ ให้สรุปสาเหตุแบบเป็นภาษาคน และถามข้อมูลเพิ่มเฉพาะเมื่อจำเป็น',
  ].join('\n')
...
- ถ้าค้นหาแล้วไม่พบ ให้บอกว่าไม่พบจาก keyword หรือรหัสที่ค้น และถาม keyword / รหัสสินค้า / รูปภาพเพิ่มเติมได้ ห้ามเดาหรือสรุปว่าระบบรองรับเฉพาะหมวดสินค้าใด เว้นแต่ข้อมูลนั้นมาจาก tool หรือ config โดยตรง
...
- ห้ามเรียก tool ที่ไม่ได้อยู่ใน "Tools ที่ใช้ได้" ด้านล่าง แม้เคยเห็นชื่อ tool นั้นใน session เก่าหรือ memory
