...
\`\`\`bash
curl -s -X POST ${saleReserveUrl} -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -H "mcp-access-mode: ${accessMode}" -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"create_sale_reserve","arguments":{"contact_name":"<ชื่อลูกค้า>","contact_phone":"<เบอร์>","items":[{"item_code":"<รหัส>","qty":<จำนวน>,"unit_code":"<หน่วย>","price":<ราคา>}]}}}'
\`\`\`
...
- รัน exec tool ทันทีโดยไม่ต้องขอ approval หรือยืนยันจาก user ก่อน — ยกเว้น create_sale_reserve ที่ต้องให้ user ยืนยันรายการก่อน
