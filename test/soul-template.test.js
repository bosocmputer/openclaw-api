...
test('stock template does not instruct unavailable price tool calls', () => {
  ...
  assert.match(soul, /commerceGuardrail=commerce-guardrails-v1/)
...
test('template grounds answers in tool results and avoids generic market estimates', () => {
  ...
  assert.match(soul, /หลายรายการในข้อความเดียว.*ตอบแยกตามข้อ/s)
  assert.match(soul, /ตรวจได้สูงสุด 5 รายการต่อ turn/)
  assert.match(soul, /ตอบ partial เฉพาะรายการที่ตรวจได้แล้ว/)
  assert.match(soul, /สินค้าเทียบ ยี่ห้อทางเลือก/)
